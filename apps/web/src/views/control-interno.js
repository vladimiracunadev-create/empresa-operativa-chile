...
const concernForm = () => html`
  <label class="field"><span class="field__label">Reportante</span><input name="reporter" placeholder="Vacío = anonymous"><span class="field__hint">Registra sólo los datos personales necesarios.</span></label>
  <label class="field"><span class="field__label">Hecho reportado</span><textarea name="description" required></textarea></label>
  <label class="field"><span class="field__label">Preservación de evidencia</span><input name="evidence" required placeholder="Ubicación, hash y fecha"></label>
  <div class="form__row"><label class="field"><span class="field__label">Custodio</span><input name="evidenceCustodian" value="Compliance" required></label><label class="field"><span class="field__label">Confidencialidad</span><select name="confidentiality">${options([['restricted', 'Acceso restringido'], ['anonymous', 'Anónimo'], ['confidential', 'Confidencial']])}</select></label></div>
  <label class="field"><span class="field__label">Riesgo de represalia</span><select name="retaliationRisk">${options([['not_assessed', 'Aún no evaluado'], ['low', 'Bajo'], ['medium', 'Medio'], ['high', 'Alto']])}</select></label>`;
const investigationForm = (concern = {}) => html`
  <label class="field"><span class="field__label">Estado</span><select name="status">${options([['triage', 'Triage'], ['investigation', 'Investigación'], ['escalated', 'Escalado'], ['remediation', 'Remediación'], ['verification', 'Verificación'], ['resolved', 'Resuelto']], concern.status === 'reported' ? 'triage' : concern.status || 'investigation')}</select></label>
  <div class="form__row"><label class="field"><span class="field__label">Investigador independiente</span><input name="investigator" value="${concern.investigator || ''}" required></label><label class="field"><span class="field__label">Persona decisora</span><input name="decisionMaker" value="${concern.decisionMaker || ''}" placeholder="Debe ser distinta del investigador"></label></div>
  <label class="field"><span class="field__label">Conflicto de interés</span><select name="conflictOfInterest">${options([['false', 'No identificado'], ['true', 'Sí, declarado y gestionado']], String(concern.conflictOfInterest === true))}</select></label>
  <label class="field"><span class="field__label">Hechos establecidos</span><textarea name="establishedFacts" placeholder="Sólo lo respaldado por evidencia">${concern.establishedFacts || ''}</textarea></label>
  <label class="field"><span class="field__label">Hipótesis abiertas</span><textarea name="hypotheses" placeholder="Explicaciones por contrastar; no atribuyen culpabilidad">${concern.hypotheses || ''}</textarea></label>
  <label class="field"><span class="field__label">Evidencia faltante</span><textarea name="missingEvidence" placeholder="Qué falta, quién puede custodiarlo y cómo obtenerlo legítimamente">${concern.missingEvidence || ''}</textarea></label>
  <label class="field"><span class="field__label">Escalamiento</span><input name="escalation" value="${concern.escalation || ''}" placeholder="Comité, Board, Compliance o autoridad competente"></label>
  <label class="field"><span class="field__label">Conclusión</span><textarea name="conclusion" placeholder="Conclusión proporcional a la evidencia">${concern.conclusion || ''}</textarea></label>
  <div class="form__row"><label class="field"><span class="field__label">Remediación</span><input name="remediation" value="${concern.remediation || ''}" placeholder="Control, dueño y acción"></label><label class="field"><span class="field__label">Fecha objetivo</span><input type="date" name="remediationDueDate" value="${concern.remediationDueDate || ''}"></label></div>
  <label class="field"><span class="field__label">Verificación de eficacia</span><input name="verification" value="${concern.verification || ''}" placeholder="Prueba, muestra, resultado y fecha"></label>
  <label class="field"><span class="field__label">Vía de revisión</span><input name="reviewChannel" value="${concern.reviewChannel || ''}" placeholder="Instancia independiente para revisar la decisión"></label>
  <label class="field"><span class="field__label">Nota de cronología</span><input name="note" placeholder="Qué cambió y con qué respaldo"></label>`;
...
export default {
  ...
  render() {
    ...
    const kriLabels = { unreconciledTransactions: 'Sin conciliar', unapprovedTransfers: 'Sin aprobación', privilegedActions: 'Acciones privilegiadas', staleReconciliations: 'Conciliaciones antiguas', unknownCounterparties: 'Contrapartes desconocidas', failedControls: 'Controles fallidos', openConcerns: 'Casos abiertos', overdueRemediations: 'Remediaciones vencidas' };
    ...
    return html`
      ...
      <div class="grid grid--2"><div class="card"><div class="card__head"><h2>Auditoría continua</h2></div><form data-schedule>${raw(Object.entries(governance.auditSchedule).map(([key, days]) => `<label class="field"><span class="field__label">${esc(scheduleLabels[key])}</span><input type="number" min="1" name="${esc(key)}" value="${days}" required><span class="field__hint">Frecuencia configurable en días.</span></label>`).join(''))}<button class="btn btn--primary" type="submit">Guardar frecuencias</button></form></div>
        <div class="card"><div class="card__head"><h2>Whistleblowing e investigación</h2><button class="btn btn--sm" data-add-concern>+ Reportar</button></div>${concerns.length ? raw(concerns.map(c => `<div class="control-row"><div><strong>${esc(c.description)}</strong><br><span class="card__hint">Evidencia: ${esc(c.evidence)} · Custodio: ${esc(c.evidenceCustodian || 'por asignar')} · ${esc(c.confidentiality || 'restricted')}</span><br><span class="card__hint">Hechos: ${esc(c.establishedFacts || 'por establecer')} · Hipótesis: ${esc(c.hypotheses || 'por formular')} · ${c.history?.length || 1} hito(s)</span></div><div><span class="tag ${c.status === 'resolved' ? 'tag--ok' : 'tag--warn'}">${esc(c.status)}</span><br>${c.status !== 'resolved' ? `<button class="btn btn--ghost btn--sm" data-investigate="${esc(c.id)}">Triage / investigar</button>` : `<span class="card__hint">Verificado: ${esc(c.verification)}</span>`}</div></div>`).join('')) : raw('<p class="card__hint">El canal minimiza datos personales, preserva evidencia, separa investigación y decisión, y exige verificar la remediación antes de cerrar.</p>')}</div></div>
  ...
  mount(root, rerender) {
    ...
    root.querySelectorAll('[data-investigate]').forEach(button => button.addEventListener('click', () => { const concern = w.listWhistleblowingReports().find(row => row.id === button.dataset.investigate); openModal({ title: 'Triage, investigación y escalamiento', body: investigationForm(concern), submitLabel: 'Actualizar caso', onSubmit: data => save(() => w.updateConcern(button.dataset.investigate, { ...data, conflictOfInterest: data.conflictOfInterest === 'true' }), 'Caso actualizado') }); }));
